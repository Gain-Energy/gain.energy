...
function KeyFeaturesPopup({activeFeature, onUpdateFeature}) {
    const pathData = "M1,0 V1 H0 V0.1 H0.354 C0.362,0.1,0.371,0.095,0.379,0.086 L0.434,0.015 C0.441,0.005,0.45,0,0.459,0 H1"
    const { popupOpen, togglePopup } = usePopup()
    const [activeId, setActiveId] = useState(activeFeature.id)

    const handleChangeFeature = (featureId) => {
        const updatedFeature = { ...featuresTexts[featureId - 1] }
    ...
    const handlePrev = () => {
        if (activeId > 1) {
            handleChangeFeature(activeId - 1)
        }
    }

    const handleNext = () => {
        if (activeId < featuresTexts.length) {
            handleChangeFeature(activeId + 1)
        }
    }
    ...
    return (
        <S.Wrapper>
            ...
            <ClipPathSVG id="inner" pathData={pathData} />
            <ClipPathSVG id="outer" pathData={pathData} />
            <S.Inner>
                <S.InnerLeft>
                ...
                <S.InnerRight>
                    <S.Heading>{activeFeature.header}</S.Heading>
                    <S.Image src={activeFeature.imageSrc}/>
                    <S.ImagePreviewBlock>
                        {featuresTexts.map(item => (
                            <S.ImagePreview 
                                key={item.id} 
                                src={item.imageSrc} 
                                onClick={() => handleChangeFeature(item.id)}
